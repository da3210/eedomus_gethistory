...
if rows is not None:
    for row in rows:
        ...
        for action in data2:
            ...
            cursor.execute("IF NOT EXISTS(SELECT * FROM [dbo].[History] WHERE periph_id = '"+row.periph_id+"' AND date = '"+date+"') INSERT INTO [dbo].[History] ([periph_id],[date],[value]) VALUES ('"+row.periph_id+"','"+date+"','"+value+"')")
            cursor.commit()
        time.sleep(.600)
else:
